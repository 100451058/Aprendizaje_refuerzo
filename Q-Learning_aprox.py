...
class Manager:
    ...
    def select_goal(self, state):
        #Actualizar epsilon
        self.epsilon = self.epsilon *0.99 if self.epsilon > 0.10 else 0.1
...
# Esperar a que el usuario introduzca la techa 'Q'
input = input("Presiona 'Q' cuando estes listo para ver el agente en acción...")
# Repetir los pasos anteriores para los estados comienzos buenos
for comienzo in comienzos_buenos:
    _ = env.reset(comienzo)
    done = False
    state = env.get_current_position()
    goal = manager.select_goal(state)  # El manager selecciona el objetivo al inicio del episodio
    movimientos = 0
