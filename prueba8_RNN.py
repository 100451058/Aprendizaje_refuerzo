...
while not done:
    ...
    action_logits, worker_hidden = worker(state.detach(), goal.detach(), worker_hidden.detach())
    ...
    if current_position == goal_position:
        ...
        if not coins:
            ...
            goal_position = next(iter(coins))  # Select a new goal  # Remove the coin from the list
    ...
    goal_reward = torch.tensor([reward], dtype=torch.float32).repeat(goal_dim)  # Match goal dimensions
